  compute_num.numeric <- function(number, smallest_prefix = "y") {  
    ...
    amount[as.numeric(number)==0] <- 0
    
    ## NA and NaN number
    amount[is.na(number)] <- NA_real_
    amount[is.nan(number)] <- NaN
    prefix[is.na(number)] <- "" # Includes NaN as well
  ...
  compute_num.units <- function(number, smallest_prefix = "y") {  
    # test if numerator is not linear unit and exit with error
    stopifnot("pretty_num() doesn't handle non-linear units" = all(table(attr(number,"units")$numerator)<2 ))
    ...
    ## Change unit with majority prefix if convertible, and change exponent accordingly
    ...
    amount[as.numeric(number)==0] <- units::set_units(0, number_unit, mode = "standard")
  ...
  pretty_num_default <- function(number, sep) {
    ...
    UseMethod("pretty_num_default")
  }
  pretty_num_default.numeric <- function(number, sep) {
  ...
  pretty_num_default.units <- function(number, sep) {
    szs <- compute_num(number)
    amt <- szs$amount
    ...
    res[!int] <- sprintf("%.2f", ifelse(szs$negative[!int], -1, 1) * amt[!int])
    sep <- ifelse(is.na(res), NA_character_, sep)
    pretty_num <- paste0(res, sep, szs$prefix, units::make_unit_label("", amt, parse=FALSE))
    # remove units added space if any
    sub(paste0("(?<=\\d",sep,")\\s")[1],"", format(pretty_num, justify = "right"), perl=TRUE)
  }
